...
export async function parseMealWithAI(rawText) {
  return createStructuredMeal([
    {
      role: "system",
      content: "You parse meal log messages into JSON. Extract every food item. If the user says a known preset like smoothie, use one item named Dad Smoothie. Use realistic nutrition estimates only when the food may not exist in the user's saved food database. If quantity is vague, choose a normal serving and mark confidence low. Do not give medical advice."
    },
