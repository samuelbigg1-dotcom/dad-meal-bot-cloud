...
import { parseMealWithAI, validateRecommendationsWithAI, explainRecommendationsWithAI, scanNutritionLabelWithAI, scanBarcodeImageWithAI } from "./ai.js";
import { calculateItems, totalItems } from "./nutrition.js";
import { generateRecommendations, remainingMacros } from "./recommendations.js";
import { getRecommendedMeals, setRecommendedMeals, sanitizeRecommendedMeals, mealSlotLabel } from "./recommendedMeals.js";
...
function currentMealSlot(slots) {
  ...
  if (hour < 16 && slots.includes("lunch")) return "lunch";
  if (hour < 21 && slots.includes("dinner")) return "dinner";
  if (slots.includes("snack")) return "snack";
  return slots[slots.length - 1] || "dinner";
}

function timingTargetCalories({ totals, goals, recommendedMeals, slot }) {
  const caloriesLeft = Math.max(0, Number(goals.calories || 0) - Number(totals.calories || 0));
  const fewSlots = recommendedMeals.length <= 2;
  if (slot === "breakfast") return Math.min(850, Math.max(450, caloriesLeft * 0.32));
  if (slot === "lunch") return Math.min(850, Math.max(500, caloriesLeft * 0.45));
  if (slot === "snack") return Math.min(450, Math.max(200, caloriesLeft * 0.25));
  return Math.min(1050, Math.max(650, caloriesLeft * (fewSlots ? 0.82 : 0.65)));
}

function applyMealTiming(options, { totals, goals, recommendedMeals, slot }) {
  const target = timingTargetCalories({ totals, goals, recommendedMeals, slot });
  const fewSlots = recommendedMeals.length <= 2;
  return [...options].map((option) => {
    const calories = Number(option.totals?.calories || 0);
    const distance = Math.abs(calories - target);
    const dinnerBonus = fewSlots && slot === "dinner" && calories >= 650 ? 160 : 0;
    const smallDinnerPenalty = fewSlots && slot === "dinner" && calories < 450 ? 260 : 0;
    return { ...option, score: round1(Number(option.score || 0) - distance * 0.25 + dinnerBonus - smallDinnerPenalty) };
  }).sort((a, b) => Number(b.score || 0) - Number(a.score || 0));
}

function recommendedMealCheckboxes(selected = []) {
  const options = [["breakfast", "Breakfast"], ["lunch", "Lunch"], ["dinner", "Dinner"], ["snack", "Snack"]];
  const set = new Set(selected);
  return `<div class="meal-slot-grid">${options.map(([value, label]) => `<label><input type="checkbox" name="recommendedMeals" value="${value}" ${set.has(value) ? "checked" : ""}> ${label}</label>`).join("")}</div>`;
}
...
function confirmMealHtml({ selectedMealType, items, mealTotals, payload, action = "/log/confirm", editHref = "/log" }) {
  return `<section class="card">
    ...
    <p class="muted">Take a quick look before saving. If anything looks off, you can edit it first.</p>
    <div class="list">
      ${items.map((item) => {
        const score = confidencePercent(item);
        const note = String(item.note || "").replace(/^Matched known food:\s*/i, "Matched saved food: ").replace(/Used base portion because unit did not match:.*/i, "Using saved serving size.");
        return `<div class="list-row" data-confidence-percent="${score}"><div><strong>${escapeHtml(item.food_name)}</strong><p>${escapeHtml(item.quantity)} ${escapeHtml(item.unit)} • ${score}% confidence${note ? ` • ${escapeHtml(note)}` : ""}</p></div><div class="cal">${round0(item.calories)} cal</div></div>`;
      }).join("")}
    </div>
...
app.get("/recommendations", requireAuth, async (req, res) => {
  ...
  const recommendedMeals = await getRecommendedMeals(USER_ID);
  const slot = currentMealSlot(recommendedMeals);
  const foods = await getRecommendationFoods();
  const rawOptions = applyMealTiming(generateRecommendations({ totals, goals, foods, count: 12 }), { totals, goals, recommendedMeals, slot });
  let options = rawOptions;
  try {
    const validation = await validateRecommendationsWithAI({ options: rawOptions });
    const byIndex = new Map(validation.map((v) => [Number(v.index), v]));
    const filtered = rawOptions.map((option, index) => {
      const check = byIndex.get(index);
      if (check && check.is_realistic === false) return null;
      return check ? { ...option, title: check.title || option.title, reason: check.reason || option.reason } : option;
    }).filter(Boolean);
  ...
  const slotCopy = `Planning for ${mealSlotLabel(slot)} based on your usual meals: ${recommendedMeals.map(mealSlotLabel).join(", ")}.`;
  res.send(layout({ title: "Recommendations", active: "recommend", user, body: `<section class="card hero"><h2>What to eat next</h2><p>Simple meal ideas from foods you already have. Built to help you hit your goals more naturally.</p><p class="muted">${escapeHtml(slotCopy)}</p>${totalsLine(remaining)}</section>${options.length ? options.map((option, idx) => { const explanation = aiExplanations[idx]?.explanation || "This is a strong fit for where you are today. It helps close your protein gap without pushing calories too high."; const note = aiExplanations[idx]?.coaching_note || ""; const payload = postMealPayload({ parsedMeal: { meal_type: slot, items: [] }, items: option.items, mealTotals: option.totals, rawMessage: `Recommended meal: ${option.title}`, mealDate: date }); return `<section class="card rec-card"><div class="section-head"><h2>${idx === 0 ? "Best fit for now" : `Option ${idx + 1}`}</h2><span>${round0(option.totals.calories)} cal</span></div><h3>${escapeHtml(option.title)}</h3><div class="list compact">${option.items.map((item) => `<div class="list-row"><div><strong>${escapeHtml(item.food_name)}</strong><p>${escapeHtml(item.quantity)} ${escapeHtml(item.unit)}</p></div><div class="cal">${round0(item.calories)} cal</div></div>`).join("")}</div>${totalsLine(option.totals)}<p>${escapeHtml(explanation)}</p>${note ? `<p class="muted">${escapeHtml(note)}</p>` : ""}<form method="post" action="/log/confirm"><input type="hidden" name="payload" value="${payload}" /><button class="button primary" type="submit">Log this meal</button></form></section>`; }).join("") : `<section class="card">${flash("Add more available foods on the Foods page to generate recommendations.", "error")}</section>`}` }));
});

app.get("/foods", requireAuth, async (req, res) => {
  const user = await currentUser();
  const foods = await getFoods();
  res.send(layout({ title: "Foods / Fridge", active: "foods", user, body: `<section class="card hero"><h2>Foods</h2><p>Keep your regular foods here for faster logging and better meal ideas.</p></section><section class="card"><h2>Scan Nutrition Facts label</h2><p class="muted">Take or upload a clear photo of the Nutrition Facts label.</p><form id="labelScanForm" class="grid-form"><input class="input wide" id="labelImageInput" type="file" accept="image/*" capture="environment" required /><button class="button primary wide" type="submit">Scan nutrition label</button></form><p class="muted" id="labelScanStatus"></p></section><section class="card"><h2>Scan barcode</h2><p class="muted">Take or upload a clear photo of the barcode. We’ll look up the packaged food so you can check it before saving.</p><form id="barcodeImageScanForm" class="grid-form"><input class="input wide" id="barcodeImageInput" type="file" accept="image/*" capture="environment" required /><button class="button primary wide" type="submit">Scan barcode photo</button></form><p class="muted" id="barcodeImageScanStatus"></p><hr class="soft-divider" /><p class="muted">Or type the barcode number manually.</p><form method="post" action="/foods/barcode" class="grid-form" id="barcodeLookupForm"><input class="input wide" id="barcodeInput" name="barcode" placeholder="Barcode / UPC number" required /><button class="button primary wide" type="submit">Look up barcode</button></form></section><section class="card"><h2>Add food manually</h2><p class="muted">Use this for homemade foods or regular items you want to save once and reuse.</p><form method="post" action="/foods" class="grid-form"><input class="input" name="name" placeholder="Name" required /><input class="input" name="baseQty" placeholder="Base qty" type="number" step="0.001" required /><input class="input" name="baseUnit" placeholder="Unit, e.g. oz/cup" required /><input class="input" name="calories" placeholder="Calories" type="number" step="0.1" required /><input class="input" name="protein" placeholder="Protein g" type="number" step="0.1" required /><input class="input" name="carbs" placeholder="Carbs g" type="number" step="0.1" required /><input class="input" name="fat" placeholder="Fat g" type="number" step="0.1" required /><input class="input" name="sugar" placeholder="Sugar g" type="number" step="0.1" required /><input class="input" name="fiber" placeholder="Fiber g" type="number" step="0.1" required /><input class="input" name="category" placeholder="Category: protein/carb/fat/vegetable/fruit" /><input class="input wide" name="aliases" placeholder="Aliases, comma separated" /><button class="button primary wide" type="submit">Save food</button></form></section><section class="card"><h2>Saved foods</h2><p class="muted">Mark foods as available so meal ideas can use them.</p><div class="food-list">${foods.map((food) => `<div class="food-row"><div><strong>${escapeHtml(food.name)}</strong><p>${escapeHtml(food.base_qty)} ${escapeHtml(food.base_unit)} • ${round0(food.calories)} cal • P ${round1(food.protein_g)} C ${round1(food.carbs_g)} F ${round1(food.fat_g)} • ${escapeHtml(food.category || "uncategorized")}</p></div><div class="food-actions"><form method="post" action="/foods/${food.id}/toggle-pantry"><button class="chip ${food.is_pantry ? "on" : "off"}" type="submit">${food.is_pantry ? "Available" : "Not available"}</button></form><form method="post" action="/foods/${food.id}/toggle-recommend"><button class="chip ${food.include_in_recommendations ? "on" : "off"}" type="submit">${food.include_in_recommendations ? "Use in meals" : "Skip for meals"}</button></form><form method="POST" action="/foods/${food.id}/delete" class="inline-form" onsubmit="return confirm('Delete this food completely? This cannot be undone.');"><button class="btn btn-danger btn-small" type="submit">Delete food</button></form></div></div>`).join("")}</div></section>` }));
});
...
app.get("/settings", requireAuth, async (req, res) => {
  const user = await currentUser();
  const recommendedMeals = await getRecommendedMeals(USER_ID);
  res.send(layout({ title: "Settings", active: "settings", user, body: `<section class="card hero"><h2>Macro goals</h2><p>Set your daily targets so meal ideas can guide you more accurately.</p></section><section class="card"><form method="post" action="/settings" class="grid-form"><label>Calories<input class="input" name="calories" type="number" value="${escapeHtml(user.calorie_goal)}" /></label><label>Protein g<input class="input" name="protein" type="number" value="${escapeHtml(user.protein_goal_g)}" /></label><label>Carbs g<input class="input" name="carbs" type="number" value="${escapeHtml(user.carbs_goal_g)}" /></label><label>Fat g<input class="input" name="fat" type="number" value="${escapeHtml(user.fat_goal_g)}" /></label><label>Sugar g<input class="input" name="sugar" type="number" value="${escapeHtml(user.sugar_goal_g)}" /></label><label>Fiber g<input class="input" name="fiber" type="number" value="${escapeHtml(user.fiber_goal_g)}" /></label><button class="button primary wide" type="submit">Save goals</button></form></section><section class="card recommended-meals-card"><h2>Recommended meals</h2><p class="muted">Choose the meals you usually eat. We’ll shape meal ideas around your routine so recommendations feel more natural and help you reach your goals.</p><form method="post" action="/settings/recommended-meals" class="stack">${recommendedMealCheckboxes(recommendedMeals)}<p class="muted">Most people can keep all four selected. If you usually skip certain meals, turn them off here.</p><button class="button primary" type="submit">Save meal routine</button></form></section>` }));
});
...
app.post("/settings/recommended-meals", requireAuth, async (req, res) => { await currentUser(); await setRecommendedMeals(USER_ID, sanitizeRecommendedMeals(bodyArray(req.body.recommendedMeals))); res.redirect("/settings"); });
