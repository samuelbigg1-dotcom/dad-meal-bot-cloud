  ...
  res.send(layout({
    ...
    body: `
      <section class="card hero">
        <h2>What did he eat?</h2>
        <p>Type it normally. The app will analyze it first, then ask you to confirm before saving.</p>
        <form method="post" action="/log/analyze" class="stack">
          <textarea class="textarea" name="mealText" rows="7" placeholder="Example: dinner 12 oz pork tenderloin, 1 cup cooked rice, 1 cup broccoli"></textarea>
    ...
    res.send(layout({
      ...
      body: `
        <section class="card">
          ...
          <p class="muted">Check the items. If something looks wrong, go back and rewrite the meal.</p>
          <div class="list">
  ...
  res.send(layout({
    ...
    body: `
      <section class="card hero">
        ...
        <p>Recommendations use foods marked “In fridge” and “Use for recommendations.”</p>
      </section>
          ...
          ${foods.map((food) => `<div class="food-row">
            ...
            <div class="food-actions">
              <form method="post" action="/foods/${food.id}/toggle-pantry">
                <button class="chip ${food.is_pantry ? "on" : ""}" type="submit">${food.is_pantry ? "In fridge" : "Not in fridge"}</button>
              </form>
              <form method="post" action="/foods/${food.id}/toggle-recommend">
                <button class="chip ${food.include_in_recommendations ? "on" : ""}" type="submit">${food.include_in_recommendations ? "Use recs" : "No recs"}</button>
              </form>
            </div>
          </div>`).join("")}
        </div>
