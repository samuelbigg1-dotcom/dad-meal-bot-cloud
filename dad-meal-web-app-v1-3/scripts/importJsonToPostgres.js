...
async function main() {
  ...
  for (const food of db.foods || []) {
    await addFood({
      ...
      is_pantry: food.is_pantry ?? true,
      include_in_recommendations: food.include_in_recommendations ?? true
    });
  }

  for (const user of db.users || []) {
    const userId = user.telegram_user_id || process.env.APP_USER_ID || "dad";
    await upsertUser({ userId, firstName: user.first_name || "Dad", timezone: user.timezone || process.env.TIMEZONE || "America/Vancouver" });
  ...
  for (const meal of db.meals || []) {
    const userId = meal.telegram_user_id || process.env.APP_USER_ID || "dad";
  ...
  for (const weight of db.weights || []) {
    const userId = weight.telegram_user_id || process.env.APP_USER_ID || "dad";
    await query(`
